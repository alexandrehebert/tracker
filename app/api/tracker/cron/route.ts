...
export async function GET(request: NextRequest) {
  ...
  try {
    const payload = await runTrackerCronJob({
      ...
      requestedBy: request.headers.get('user-agent') ?? 'vercel-cron',
    });

    return buildJsonResponse(payload);
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Unable to execute tracker cron.';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const payload = await runTrackerCronJob({
      ...
      requestedBy: typeof body.requestedBy === 'string' ? body.requestedBy : 'tracker cron dashboard',
    });
